...
const generateSqlDump = async (tables) => {
    ...
    if (validTables.length === 0)
        throw new Error('No valid tables specified');
...
const restoreSqlDump = async (sqlPath) => {
...
const safeStringify = (data) => JSON.stringify(data, jsonReplacer, 2);
// Generate DB backup ZIP (SQL dump only — fast, compact, reliable)
export const generateDbBackup = async (tables) => {
    await ensureBackupsDir();
    const validTables = tables.filter(t => ALLOWED_TABLES.includes(t));
    if (validTables.length === 0) {
...
export const generateFullBackup = async (tables) => {
    await ensureBackupsDir();
    const validTables = tables.filter(t => ALLOWED_TABLES.includes(t));
    if (validTables.length === 0) {
        throw new Error('No valid tables specified for backup');
...
export const restoreFromZip = async (zipPath, includeImages = true) => {
    await ensureBackupsDir();
...
const restoreDbData = async (tempDir, isOldFormat, tablesIncluded) => {
    if (isOldFormat) {
        ...
        tablesIncluded = backupData.metadata.tables_included.filter((t) => ALLOWED_TABLES.includes(t));
        await restoreTablesFromJson(tablesIncluded, backupData);
    }
    else {
        const validTables = tablesIncluded.filter((t) => ALLOWED_TABLES.includes(t));
        const backupData = {};
...
const restoreTablesFromJson = async (tables, backupData) => {
