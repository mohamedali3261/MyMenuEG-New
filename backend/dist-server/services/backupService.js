...
const ALLOWED_TABLES = [
    ...
    'admins', 'backup_logs', 'store_pages', 'settings', 'product_detail_items',
    'product_variants'
];
const ensureBackupsDir = async () => {
    await fs.promises.mkdir(BACKUPS_DIR, { recursive: true });
};
export const generateBackupFile = async (tables) => {
...
export const restoreFromZip = async (zipPath) => {
    await ensureBackupsDir();
    ...
    try {
        ...
        await prisma.$transaction(async (tx) => {
            ...
            const tables = backupData.metadata.tables_included.filter((t) => ALLOWED_TABLES.includes(t));
            // Truncate with validated table names
            for (const table of tables) {
                await tx.$executeRawUnsafe(`TRUNCATE TABLE ${table};`);
            }
            // Restore from JSON with validated table names
