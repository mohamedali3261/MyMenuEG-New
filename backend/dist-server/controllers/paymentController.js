...
export const handleCallback = async (req, res) => {
    try {
        ...
        if (order.email) {
            sendOrderStatusEmail(order.email, order.id, order.customer_name || 'Customer', 'processing', 'جاري التجهيز').catch(err => logger.error('Failed to send payment confirmation email', err));
        }
        // Create notification
