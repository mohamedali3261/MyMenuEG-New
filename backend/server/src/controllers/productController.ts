  ...
  try {
    ...
    const payload = await cacheResolveSWR(
      ...
      async () => {
        ...
        const variants = (product as any).product_variants.map((v: any) => ({
          ...v,
          images: v.product_variant_images.map((img: any) => img.url)
        }));

        // Image Fallback Logic
        let finalImageUrl = (product as any).image_url;
        if (!finalImageUrl && variants.length > 0) {
          const firstVariantWithImages = variants.find((v: any) => v.images && v.images.length > 0);
          if (firstVariantWithImages) {
            finalImageUrl = firstVariantWithImages.images[0];
        ...
        const formatted = {
          ...(product as any),
          image_url: finalImageUrl,
          cat_name_ar: (product as any).categories?.name_ar,
          cat_name_en: (product as any).categories?.name_en,
          specs: (product as any).product_specs,
          images: (product as any).product_images.map((img: any) => img.url || ''),
          quantity_prices: (product as any).product_quantity_prices,
          variants,
          detail_items: (product as any).product_detail_items,
          faqs: (product as any).product_faqs,
          fbt_products: (product as any).fbt_main?.map((fbt: any) => fbt.related_product).filter(Boolean) || [],
  ...
  try {
        ...
        for (const variant of input.variants) {
          ...
          if (variant.images && Array.isArray(variant.images)) {
            await tx.product_variant_images.createMany({
              data: variant.images.map((img: string) => ({
    ...
    if (err.name === 'ZodError') {
      ...
      return res.status(400).json({
        ...
        details: Array.isArray(errors) ? errors.map((e: any) => e.message) : ['Validation Error']
      });
    }
    console.error(err);
...
export const deleteProduct = async (req: Request, res: Response) => {
  try {
    ...
    // 3. Log Audit
    await logAudit('delete_product', (req as any).user?.username || 'system', `Deleted product: ${String(id)}`);
...
export const updateProductStatus = async (req: Request, res: Response) => {
  try {
    ...
    await logAudit('update_product_status', (req as any).user?.username || 'system', `Updated product status: ${id} -> ${status}`);
