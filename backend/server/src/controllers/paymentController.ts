...
export const createPayment = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const isAdminFlow = Boolean((req as any).user);
    ...
    const paymentData = {
      ...
      method: method as 'card' | 'fawry' | 'wallet',
      walletNumber
    };
    ...
    await logAudit(
      'payment_create',
      actor,
      `Order ${orderId}, method=${method}, flow=${isAdminFlow ? 'admin' : 'customer'}`
    );
...
export const handleCallback = async (req: Request, res: Response) => {
  ...
  try {
    ...
    await logAudit('payment_callback_error', 'system', `Paymob callback exception: ${(err as Error).message || 'unknown'}`);
 ...
 */
export const handleFawryCallback = async (req: Request, res: Response) => {
  let idemRecordId: number | null = null;
  const respond = async (statusCode: number, body: any) => {
    if (idemRecordId) {
      if (statusCode >= 200 && statusCode < 300) {
  ...
  try {
    ...
    await logAudit('fawry_callback_error', 'system', `Fawry callback exception: ${(err as Error).message || 'unknown'}`);
