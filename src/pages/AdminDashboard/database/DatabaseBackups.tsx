...
export default function DatabaseBackups() {
  ...
  const handleTestGhConnection = async () => {
    setIsTestingGh(true);
    try {
      const res = await fetch(`${API_BASE}/api/database/github/test`, {
        method: 'POST',
        headers: { 
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleSyncToGithubNow = async () => {
    setIsSyncingGh(true);
    try {
      const res = await fetch(`${API_BASE}/api/database/github/sync-now`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const data = await res.json();
  ...
  const fetchSavedBackups = useCallback(async () => {
    try {
      const res = await fetch(`${API_BASE}/api/database/saved-backups`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const fetchLogs = useCallback(async () => {
    try {
      const res = await fetch(`${API_BASE}/api/database/logs`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const handleDownloadBackup = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/api/database/backup?tables=${tablesParam}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const handleSaveToDisk = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/database/backup-to-disk`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
  ...
  const handleRestoreBackup = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/database/restore`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` },
        body: formData
      });
  ...
  const handleRestoreSaved = async (filename: string) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/database/saved-backups/${filename}/restore`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
  ...
  const handleDeleteSaved = async (filename: string) => {
    ...
    try {
      await fetch(`${API_BASE}/api/database/saved-backups/${filename}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      fetchSavedBackups();
  ...
  const handleDownloadSaved = (filename: string) => {
    window.open(`${API_BASE}/api/database/saved-backups/${filename}?token=${token}`, '_blank');
  };
  ...
  const handleWipeDatabase = async () => {
    ...
    setIsWiping(true);
    try {
      const response = await fetch(`${API_BASE}/api/database/wipe`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (response.ok) {
