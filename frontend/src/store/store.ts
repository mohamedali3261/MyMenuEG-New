...
interface Product {
  ...
  variants?: Array<{
    ...
    image_url?: string;
    imageUrl?: string;
    images?: string[]; // Multiple images for this variant
  }>;
...
export const useStore = create<AppState>()(
  persist(
    (set, get) => ({
      ...
      pages: [],
      isDataLoaded: false,
      isFetchingInitialData: false, // Internal guard
      isFetchingSettings: false,    // Internal guard
      ...
      fetchSettings: async (force = false) => {
        if (!force && isFetchingSettings) return get().settingsCache.data;
      ...
      addAdmin: async (data) => {
        try {
          ...
          return { success: false, error: error.response?.data?.error || 'Failed to add admin' };
        }
      },
      updateAdmin: async (id, data) => {
        try {
          ...
          return { success: false, error: error.response?.data?.error || 'Failed to update admin' };
        }
      },
