...
interface SvgMarqueeSettings {
  ...
  font_family: string;
  enabled: boolean;
  items: SvgMarqueeItem[];
}

export default function SvgMarquee() {
  const [settings, setSettings] = useState<SvgMarqueeSettings | null>(null);
  const canSize = settings?.can_size || 200;
  ...
  useEffect(() => {
    if (window.matchMedia('(prefers-reduced-motion)').matches && filtersRef.current) {
      filtersRef.current.pauseAnimations();
      setPaused(true);
    }
  }, []);
  ...
  const togglePause = () => {
    ...
    if (next) {
      ...
      filtersRef.current.unpauseAnimations();
    }
  };

  if (!settings || !settings.enabled) return null;

  const dur = `${settings.animation_duration || 15}s`;

  const hex = settings.text_color.replace('#', '');
  ...
  return (
    <div style={{ margin: 0, height: '100%', display: 'grid', position: 'relative' }}>
      {/* Pause Button */}
      <button
        onClick={togglePause}
        aria-pressed={paused}
        style={{
          ...
          background: paused ? settings.text_color : '#fff',
          color: paused ? '#fff' : settings.text_color,
          fontWeight: 'bold',
          fontSize: '1rem',
          padding: '10px 20px',
          position: 'absolute',
      ...
      {/* SVG Filters */}
      <svg ref={filtersRef} aria-hidden="true" width="10000" height="10000" style={{ position: 'fixed' }}>
        <defs>
          ...
          <filter id="marquee-left" x="0" y="0" width="600%" height="100%" primitiveUnits="objectBoundingBox">
            <feOffset dx="0">
              <animate attributeName="dx" to="-1" dur={dur} repeatCount="indefinite" />
            </feOffset>
          </filter>
          <filter id="marquee-right" x="0" y="0" width="600%" height="100%" primitiveUnits="objectBoundingBox">
            <feOffset dx="-1">
              <animate attributeName="dx" to="0" dur={dur} repeatCount="indefinite" />
            </feOffset>
          </filter>
        </defs>
      </svg>
      ...
      <div style={{
        ...
        {/* Left Column */}
        <div style={{ gridArea: '1/1/-1/2', position: 'relative', zIndex: 1 }}>
          <div className="svg-marquee">
            <div className="svg-marquee--inner">
              <p style={{ filter: 'url(#duplicate) url(#marquee-left)', color: settings.text_color, fontFamily: settings.font_family }}>
                {settings.left_text_1}
                {settings.items.filter(i => i.strip === '1' && i.type === 'image').map(item => (
                  <img key={item.id} src={resolveAssetUrl(item.image_url!)} alt="" style={{ height: '0.8em', width: 'auto', objectFit: 'contain' }} />
                ))}
              </p>
            </div>
          </div>
          <div className="svg-marquee">
            <div className="svg-marquee--inner">
              <p className="svg-marquee-outline" style={{ filter: 'url(#duplicate) url(#marquee-right)', WebkitTextStroke: `0.05em #ffffff`, color: settings.text_color, fontFamily: settings.font_family }}>
                {settings.left_text_2}
                {settings.items.filter(i => i.strip === '2' && i.type === 'image').map(item => (
                  <img key={item.id} src={resolveAssetUrl(item.image_url!)} alt="" style={{ height: '0.8em', width: 'auto', objectFit: 'contain' }} />
                ))}
              </p>
            </div>
        ...
        {/* Right Column */}
        <div style={{ gridArea: '1/2/-1/3', position: 'relative', zIndex: 1 }}>
          <div className="svg-marquee">
            <div className="svg-marquee--inner">
              <p className="svg-marquee-outline" style={{ filter: 'url(#duplicate) url(#marquee-left)', WebkitTextStroke: `0.05em #ffffff`, color: settings.text_color, fontFamily: settings.font_family }}>
                {settings.right_text_1}
                {settings.items.filter(i => i.strip === '1' && i.type === 'text').map(item => (
                  <span key={item.id}>{item.text_en || item.text_ar}</span>
                ))}
              </p>
            </div>
          </div>
          <div className="svg-marquee">
            <div className="svg-marquee--inner">
              <p style={{ filter: 'url(#duplicate) url(#marquee-right)', color: settings.text_color, fontFamily: settings.font_family }}>
                {settings.right_text_2}
                {settings.items.filter(i => i.strip === '2' && i.type === 'text').map(item => (
                  <span key={item.id}>{item.text_en || item.text_ar}</span>
                ))}
              </p>
            </div>
        ...
        {/* Can SVG */}
        {settings.show_can && !settings.can_image_url && (
          <svg role="img" className="svg-marquee-can" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 681 1271" style={{ gridArea: '1/1/-1/-1', width: '50vw', maxWidth: canSize, marginInline: 'auto', position: 'relative', zIndex: 2 }}>
      ...
      <style>{`
        ...
        .svg-marquee-outline {
          -webkit-text-stroke: 0.05em ${settings.text_color};
        }
        .svg-marquee-can {
          grid-area: 1/1/-1/-1;
          width: 50vw;
          max-width: ${canSize}px;
          margin-inline: auto;
          position: relative;
        }
        .svg-marquee-bg {
