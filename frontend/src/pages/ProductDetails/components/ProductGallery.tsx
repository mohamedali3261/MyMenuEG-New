...
export default function ProductGallery({ images, activeIndex, onImageSelect }: { images: string[], activeIndex?: number, onImageSelect?: (index: number) => void }) {
  ...
  // Reset active image when the image list content changes (only if not controlled)
  useEffect(() => {
    if (activeIndex === undefined) {
      setActive(0);
    }
  ...
  const handlePrev = (e: React.MouseEvent) => {
    e.stopPropagation();
    setActive((prev) => {
      const next = prev > 0 ? prev - 1 : validImages.length - 1;
      onImageSelect?.(next);
      return next;
    });
  }
  ...
  const handleNext = (e: React.MouseEvent) => {
    e.stopPropagation();
    setActive((prev) => {
      const next = prev < validImages.length - 1 ? prev + 1 : 0;
      onImageSelect?.(next);
      return next;
    });
  }
      ...
      >
        ...
        {validImages.length > 1 && (
          <>
            <button
               onClick={handlePrev}
               className="absolute left-4 top-1/2 -translate-y-1/2 relative p-2.5 rounded-xl bg-slate-100/80 dark:bg-black/50 backdrop-blur-md border border-slate-200 dark:border-white/10 text-slate-500 dark:text-slate-300 hover:text-primary-500 opacity-0 group-hover:opacity-100 transition-all shadow-md -translate-x-4 group-hover:translate-x-0"
            >
               <ChevronLeft size={20} />
            </button>
            <button
               onClick={handleNext}
               className="absolute right-4 top-1/2 -translate-y-1/2 relative p-2.5 rounded-xl bg-slate-100/80 dark:bg-black/50 backdrop-blur-md border border-slate-200 dark:border-white/10 text-slate-500 dark:text-slate-300 hover:text-primary-500 opacity-0 group-hover:opacity-100 transition-all shadow-md translate-x-4 group-hover:translate-x-0"
            >
