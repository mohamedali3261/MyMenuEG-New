...
export const getProducts = async (req, res) => {
    try {
        ...
        // Format output to match existing API structure
...
export const deleteProduct = async (req, res) => {
    try {
        ...
        // 3. Log Audit
        await logAudit('delete_product', req.user.username, `Deleted product: ${id}`);
...
export const updateProductStatus = async (req, res) => {
    try {
        ...
        await logAudit('update_product_status', req.user.username, `Updated product status: ${id} -> ${status}`);
        res.json({ success: true });
    }
    catch (err) {
        console.error(err);
