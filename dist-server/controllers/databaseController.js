...
import { generateBackupFile } from '../services/backupService';
...
const resolveBackupPath = (filenameParam) => {
    ...
    const targetPath = path.resolve(normalizedBackupsDir, filename);
...
export const createManualBackup = async (req, res) => {
    try {
        ...
        await logAudit('manual_backup', req.user.username, `Saved: ${result.filename}`);
...
export const downloadBackup = async (req, res) => {
    try {
        const resolved = resolveBackupPath(String(req.params.filename));
        if (!resolved) {
...
export const deleteBackupFile = async (req, res) => {
    try {
        ...
        if (exists) {
            ...
            await logAudit('delete_backup', req.user.username, `Deleted: ${resolved.filename}`);
        }
    ...
    try {
        ...
        await prisma.$transaction(async (tx) => {
            ...
            for (const table of ALL_TABLES) {
                if (table === 'admins')
                    continue; // Don't wipe admins to avoid lockout
        ...
        await logAudit('wipe', req.user.username, "Full wipe with safety snapshot");
