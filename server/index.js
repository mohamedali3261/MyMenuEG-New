...
const JWT_SECRET = process.env.JWT_SECRET || 'mymenueg_secret_key_123';
...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    const token = jwt.sign(
      { id: admin.id, username: admin.username, is_super_admin: admin.is_super_admin, permissions: admin.permissions },
      JWT_SECRET,
      { expiresIn: '24h' }
    );
...
app.get('/api/admins', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await db.query('SELECT id, username, is_super_admin, permissions, created_at FROM admins ORDER BY created_at DESC');
...
app.post('/api/admins', authenticateToken, async (req, res) => {
  try {
    ...
    // Check if exists
    const exists = await db.query('SELECT * FROM admins WHERE username = $1', [username]);
    ...
    await db.query(
      'INSERT INTO admins (id, username, password, permissions, is_super_admin) VALUES ($1, $2, $3, $4, $5)',
      [id, username, hashedPassword, JSON.stringify(permissions || []), false]
    );
...
app.delete('/api/admins/:id', authenticateToken, async (req, res) => {
  try {
    if (!req.user.is_super_admin) return res.status(403).json({ error: 'Unauthorized' });
    if (req.params.id === 'admin-1') return res.status(400).json({ error: 'Cannot delete super admin' });
